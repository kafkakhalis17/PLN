      ...
      <div class="mb-3 card">
         <div class="card-header">
            <ul class="nav nav-justified">
                  ...
                  <li class="nav-item"><a data-toggle="tab" href="#tab-eg7-1" class="nav-link">Menunggu Pembayaran</a></li>
...
<script>
   ...
   function GetDetail(id) {
      $.ajax({
         type: 'GET',
         url: '<?= base_url('VerifikasiData/getdetailmodal/')?>'+ id,
         beforeSend: function(){
            $('.loading-modal').show();
         },
         complete: function(){
   ...
   function refreshdataverifikasi() { 
         $.ajax({
            type: 'get',
            async : false,
            url: '<?= base_url('VerifikasiData/getAllDatapermohonan')?>',
            dataType: 'json',
            beforeSend: function(){
               $('.loading-table').show();
               $('#tablesec').hide();
            ...
            success: function(datapermohonan){
               ...
               for (let i = 0; i < datapermohonan.length; i++) {
                  if (datapermohonan[i].status == "0") {
                     rowverifikasi +=  '<tr>'+
                     '<td class="text-center text-muted">#'+ datapermohonan[i].id_pmh_plgn +'</td>'+
                     '<td>'+
                        '<div class="widget-content p-0">' +
                           '<div class="widget-content-wrapper">' +
                              '<div class="widget-content-left flex2">' +
                                    '<div class="widget-heading">'+ datapermohonan[i].nama +'</div>' +
                  ...
                  '<td class="text-center">' +
                     ...
                     '</td>' +
                  '<td class="text-center">' +
                     '<button type="button" class="btn btn-primary" onclick="GetDetail('+ datapermohonan[i].id_pmh_plgn +')" data-toggle="modal" data-target="#detail">Detail</button>' +
                  '</td>' +
                  '</tr>';
                  
                  }else if (datapermohonan[i].status == "1") {
                     rowbayar += '<tr>'+
                     '<td class="text-center text-muted">#'+ datapermohonan[i].id_pmh_plgn +'</td>'+
                     '<td>'+
                        '<div class="widget-content p-0">' +
                  ...
                  '</td>' +
                  '</tr>';
                  }else if (datapermohonan[i].status == "2") {
                     rowpasang += '<tr>'+
                     '<td class="text-center text-muted">#'+ datapermohonan[i].id_pmh_plgn +'</td>'+
                     '<td>'+
                        '<div class="widget-content p-0">' +
                  ...
                  '<td class="text-center">'+ datapermohonan[i].alamat +'</td>' +
                  '<td class="text-center">' +
                     '<div class="badge badge-warning">menunggu Verifikasi Pemasangan</div>' +
                     '</td>' +
                  '<td class="text-center">' +
                     '<button type="button" class="btn btn-primary"  onclick="GetDetail('+ datapermohonan[i].id_pmh_plgn +')" data-toggle="modal" data-target="#detail">Detail</button>' +
                  '</td>' +
   ...
   function unverifikasi() {
      ...
      swal.fire({
			...
			.then((willDelete) => {
               ...
               $.ajax({
                  type: 'POST',
                  url: '<?= base_url('VerifikasiData/unverifikasi/')?>'+ id,
                  beforeSend: function(){
                     $('#detail-data').empty();
                     $('#tbody-verifikasi').empty();
                     $('.loading-modal').show();
                  ...
                  success: function (data) {
                     // window.location.href ="<?= base_url('VerifikasiData/verifikasipasangbaru')?>";
                  }
               });
               return false;
				} else {
   ...
   function verifikasi() {
      ...
      swal.fire({
			...
			.then((willDelete) => {
               ...
               $.ajax({
                  type: 'POST',
                  url: '<?= base_url('VerifikasiData/verifikasipemasangan/')?>'+ id,
                  beforeSend: function(){
                     $('#detail-data').empty();
                     $('#tbody-verifikasi').empty();
                     $('.loading-modal').show();
